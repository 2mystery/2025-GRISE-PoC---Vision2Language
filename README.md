...
> **G-RISE 2025 지산학 얼라이언스 고도화 지역 산업군 PoC 연구 (Vision2Language 팀)** <br>
...
현대 제조 및 물류 현장에서는 인간과 로봇이 공간을 공유하며 협업합니다. 하지만 기존의 로봇 인지 시스템은 고정된 명령어에만 의존하여, **"오른쪽의 검은색 귀덮개를 치워주세요"**와 같은 유동적이고 복잡한 자연어 지시를 처리하는 데 한계가 있습니다.

본 프로젝트는 자연어(Text)와 시각 정보(Image)를 동시에 이해하는 **MDETR(Modulated DETR)** 모델을 도입하여, 작업자의 복합적인 자연어 명령을 이해하고 해당 객체의 위치를 정확히 파악(Visual Grounding)하는 **Vision-Language 객체 탐지 파이프라인의 현장 적용 가능성(PoC)**을 검증합니다.
...
본 연구는 CNN과 Transformer를 결합한 End-to-End 멀티모달 모델인 **MDETR**을 채택했습니다.
...
## 🔥 4. Key Technical Contributions (주요 문제 해결 과정)
> **💡 포트폴리오 핵심:** 이질적인 산업용 데이터셋을 모델에 적용하기 위해 **데이터 로딩 및 학습 파이프라인을 독자적으로 재설계**했습니다.

### 1️⃣ Custom Collate Function 구현 (차원 불일치 해결)
...
* **Solution:** 디렉터리 전수 스캔 및 리매핑(Re-mapping) 스크립트를 적용하여 데이터 로딩 오류 100% 해결.
...
> ![Detection Result 1](./images/result1.png)
> ![Detection Result 2](./images/result2.png)
...
이신비 (컴퓨터공학부) - Project Lead, Pipeline Engineering
노현두 (컴퓨터공학부) - Model Optimization
조민채 (컴퓨터공학부) - Data Preprocessing & Evaluation
